...
@section('title', 'Master Bahan Kitchen')

@push('styles')
    <style>
        /* Modern Professional Styles */
        .master-kitchen-container {
            padding: 20px;
...
@section('content')
    <div class="master-kitchen-container">
        <!-- Filter Card -->
        <div class="filter-card">
            <h3><i class="fas fa-filter"></i> Filter Data</h3>
            <form method="GET" action="{{ route('master-kitchen.index') }}" class="filter-form">
                ...
                <div class="form-group filter-buttons">
                    ...
                    <a href="{{ route('master-kitchen.index') }}" class="btn btn-secondary">
        ...
        <div class="main-card">
            ...
            <div class="card-header">
                <div class="header-title">
                    ...
                    <div>
                        <h3>Master Bahan Kitchen</h3>
                        <p>Kelola semua bahan master kitchen dengan mudah</p>
                    </div>
                </div>
            ...
            <div class="card-body">
                ...
                @if ($masterKitchen->isEmpty())
                    <div class="empty-state">
                        <i class="fas fa-utensils"></i>
                        <h4>Belum ada data master bahan kitchen</h4>
                        <p>Mulai dengan menambahkan bahan baru untuk mengelola stok kitchen Anda</p>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-plus-circle"></i> Tambah Master Bahan Kitchen</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <form action="{{ route('master-kitchen.store') }}" method="POST">
                    @csrf
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-control" style="background: #e8f5e9; color: #2e7d32; font-weight: 500;">
                                    <i class="fas fa-spinner fa-spin"></i> Auto generate (MAxxx)
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="satuan-input-group">
                                    <input type="text" class="form-control @error('nama_satuan') is-invalid @enderror"
                                        id="satuanInputCreate" value="{{ old('nama_satuan') }}"
                                        placeholder="Klik untuk memilih satuan" readonly required
                                        onclick="showSatuanModal('create')">
                                    <button type="button" class="btn-select" onclick="showSatuanModal('create')">
                                        <i class="fas fa-search"></i>
                                    </button>
                                    <input type="hidden" name="nama_satuan" id="selectedSatuanCreate"
    ...
    @foreach ($masterKitchen as $item)
        <div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1"
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"><i class="fas fa-edit"></i> Edit Master Bahan Kitchen</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    <form action="{{ route('master-kitchen.update', $item->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="satuan-input-group">
                                        <input type="text" class="form-control"
                                            id="satuanInputEdit{{ $item->id }}" value="{{ $item->nama_satuan }}"
                                            readonly required onclick="showSatuanModal('edit', {{ $item->id }})">
                                        <button type="button" class="btn-select"
                                            onclick="showSatuanModal('edit', {{ $item->id }})">
                                            <i class="fas fa-search"></i>
                                        </button>
                                        <input type="hidden" name="nama_satuan"
                                            id="selectedSatuanEdit{{ $item->id }}" value="{{ $item->nama_satuan }}">
                                    </div>
                                </div>
        ...
        <div class="modal fade" id="deleteModal{{ $item->id }}" tabindex="-1"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('master-kitchen.destroy', $item->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
    ...
    <!-- Satuan Modal -->
    <div class="modal fade modal-satuan" id="satuanModal" tabindex="-1" aria-labelledby="satuanModalLabel"
        aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="search-satuan">
                        <i class="fas fa-search"></i>
                        <input type="text" class="form-control" id="searchSatuan" placeholder="Cari satuan...">
                    </div>
                    <ul class="satuan-list" id="satuanList">
                        @foreach ($satuan as $s)
                            <li class="satuan-item" data-value="{{ $s->nama_satuan }}"
                                onclick="selectSatuan('{{ $s->nama_satuan }}')">
    ...
    <div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                ...
                <form action="{{ route('master-kitchen.export') }}" method="POST" id="exportForm">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                        ...
                        <div class="alert alert-info" style="background: #e3f2fd; border-color: #2196f3;">
                            ...
                            <br>
                            <small class="text-muted">Master Stok Kitchen - Tanggal - Bulan - Tahun.xlsx</small>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
...
@push('scripts')
    <script>
        ...
        // Show satuan modal
        function showSatuanModal(type, id = '') {
            ...
            // Show all items
            document.querySelectorAll('#satuanList .satuan-item').forEach(item => {
            ...
            setTimeout(() => {
                let selectedValue = '';
                if (type === 'create') {
                    selectedValue = document.getElementById('selectedSatuanCreate')?.value || '';
                } else if (type === 'edit' && id) {
                    selectedValue = document.getElementById('selectedSatuanEdit' + id)?.value || '';
                }
        ...
        // Select satuan
        function selectSatuan(satuan) {
            if (currentModalType === 'create') {
                document.getElementById('satuanInputCreate').value = satuan;
                document.getElementById('selectedSatuanCreate').value = satuan;
            } else if (currentModalType === 'edit' && currentItemId) {
                document.getElementById('satuanInputEdit' + currentItemId).value = satuan;
                document.getElementById('selectedSatuanEdit' + currentItemId).value = satuan;
            }
        ...
        document.getElementById('searchSatuan')?.addEventListener('input', function(e) {
            ...
            const satuanItems = document.querySelectorAll('#satuanList .satuan-item');
        ...
        document.getElementById('satuanModal')?.addEventListener('show.bs.modal', function() {
            // Find all open modals and set their z-index lower
            document.querySelectorAll('.modal.show').forEach(modal => {
                if (modal.id !== 'satuanModal') {
